...
public class LayoutTiles : MonoBehaviour {
	...
	public void BuildRoom(PT_XMLHashtable room){
		...
		//This keeps the Mage from accidentally triggering OnTriggerExit() on a Portal.
		//In my testing, I found that OnTriggerExit was being called at strange times.
		...
		//Split tje room into rows of tiles based on carriage returns in the 
		//Rooms.xml file
			...
			for(int x=0; x<roomRows[y].Length; x++){
				...
				case "X":
					...
					if(firstRoom){
						...
						roomNumber = rNumStr;
						//Setting roomNumber now keeps and portals from moving the Mage to them in this first room.
						firstRoom = false;
					}
					break;
				...
				case "F":
					...
					//Attaching this to the tileAnchor means that the Portal will be Destroyed when a new room is built
					p.toRoom = rawType;
					portals.Add (p);
					break;
		...
		foreach (Portal p in portals) {
			...
			//Alternatively, if firstRoom ==true and there was no X in the room
			//(as a default Mage starting point), move the Mage to this Portal
			//as a backup measure (if, for instance, you want to just load room
			//number "5")
			if (p.toRoom == roomNumber || firstRoom){
				...
				//_Mage maintains her facing from the previous room, so there is no need to rotate her in order
				//for her to enter this room facing the right direction.
				p.justArrived = true;
